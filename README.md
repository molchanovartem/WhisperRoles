...
| `CPU_THREADS` | 50% ядер | Потоки CTranslate2. `0` = половина ядер. Задай явно если нужен контроль |
...
| `large-v3-turbo` | ~1.5 GB | ~6 GB | Медленно | Близко к максимальному |
...
| Рядом с файлом появился `.error` | Обработка упала. Внутри `.error` файла — текст ошибки. Удали `.error` чтобы повторить |
| После краша контейнера файл завис | При перезапуске зависшие `.processing` маркеры удаляются автоматически |
